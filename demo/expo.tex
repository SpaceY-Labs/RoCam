...
\block{System Architecture}{
  ...
  % --- Top Section: Diagram ---
  % I kept your specific trim and clip settings, but increased the width to 90%
  \posterimg[
    width=0.9\linewidth,
    trim=40 420 40 420,
    clip
  ...
  \vspace{1.5em} % Breathing room between the diagram and text

  % --- Bottom Section: Bullet Points ---
  \small
  \begin{itemize}[leftmargin=2em, itemsep=0.6em]
    \item \textbf{Core Processing:} Runs a distributed multi-process architecture on an NVIDIA Jetson.
    \item \textbf{Vision Pipeline:} A GStreamer pipeline captures camera input at 1920$\times$1080 and feeds it through NVIDIA DeepStream for real-time YOLO inference.
    \item \textbf{Gimbal Control:} Detection results drive a proportional tracking controller commanding a two-axis servo gimbal via an STM32 microcontroller over UART.
    \item \textbf{Web Backend:} A Flask-based control process manages state, recording, transcoding, and serves a REST API with SSE to the web frontend.
  \end{itemize}
