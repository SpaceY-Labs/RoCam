...
\begin{tabularx}{\textwidth}{llX}
  ...
  Apr. 5, 2026             & All team members        & Rev 1: Address TA/instructor feedback, fix typos and grammar, add traceability to Hazard Analysis safety requirements (SR1--SR4), improve requirement clarity \\
  \bottomrule
\end{tabularx}
...
The client of this project is the McMaster Rocketry Team. They are the primary
end users who will deploy the system during launches.
...
streaming equipment. The live stream technician is assumed to have a high level
of familiarity with live streaming software and hardware.
...
Throughout the development process, the development team will continue to
...
\begin{itemize}[leftmargin=*]
  \item[BC-1] \emph{The project shall not exceed the budget of 500 CAD.}\\[2mm]
        \textbf{Rationale:} Limited by the capstone project requirement.\\
        \textbf{Fit Criterion:} The sum of the costs of the hardware components required for the project must be less than 500 CAD.\\
        \textbf{Source PUC:} all\\
        \textbf{Priority:} High
\end{itemize}
...
\begin{enumerate}
  \item The camera operator connects the camera to the live streaming equipment.
\end{enumerate}
...
\textbf{Main Flow of Steps:}
\begin{enumerate}
  \item The camera operator connects the system to the live streaming equipment.
\end{enumerate}

\textbf{Outcome:} The live streaming equipment receives a video feed from the system.
...
\begin{itemize}[leftmargin=*]
  \item[LR-1] \emph{The system shall be easy to learn for new users.}\\[2mm]
        ...
        \textbf{Fit Criterion:} New users with a reasonable level of technological familiarity who have read the manual are able to successfully perform all product use cases within 20 minutes without external assistance.\\
        \textbf{Source PUC:} 2.1, 2.2, 2.3, 4.1, 4.2, 5 \\
        \textbf{Priority:} High
...
\begin{itemize}[leftmargin=*]
  \item[ACR-1] \emph{Status indicators shall remain distinguishable under common
...
\begin{itemize}[leftmargin=*]
  \item[SCR-1] \emph{The system must allow the user to manually exit armed or tracking
        ...
        \textbf{Rationale:} Manual override is necessary to ensure safety and provide user control in unexpected circumstances.\\
        \textbf{Fit Criterion:} When the user issues an exit command during armed or tracking mode, the system transitions to idle mode within 0.5 seconds and ceases any automated gimbal movement.\\
        \textbf{Source PUC:} 4.2 \\
        \textbf{Priority:} High \\
        \textbf{HA Trace:} SR1 (angular velocity limit enforcement), SR4 (emergency stop)
  ...
  \item[SCR-2] \emph{The system must be hands-off (autonomous) from rocket ignition
        ...
        \textbf{Source PUC:} 3 \\
        \textbf{Priority:} High \\
        \textbf{HA Trace:} SR2 (PPE -- reduces need for physical proximity during autonomous flight), SR3 (self-correcting tracking)

  \item[SCR-3] \emph{The system must be remotely operable over a local network.}\\[2mm]
        ...
        \textbf{Source PUC:} all \\
        \textbf{Priority:} High \\
        \textbf{HA Trace:} SR2 (PPE -- remote operation reduces operator proximity to gimbal hardware)
  ...
  \item[SCR-4] \emph{The system must only enter armed mode when explicitly instructed
        ...
        \textbf{Priority:} High \\
        \textbf{HA Trace:} SR1 (prevents uncontrolled gimbal movement), SR4 (emergency stop available from any state)
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item[RFR-2] \emph{The system shall immediately stop all automated operations and
        ...
        \textbf{Source PUC:} all \\
        \textbf{Priority:} High \\
        \textbf{HA Trace:} SR1 (prevents gimbal damage), SR4 (emergency stop safe state)
...
\begin{itemize}[leftmargin=*]
  \item[INT-1] \emph{The system must output a live video stream via HDMI.}\\[2mm]
        \textbf{Rationale:} The customer's live streaming equipment accepts HDMI as the integration standard, alternative connectors would require additional adapters.\\
...
\begin{enumerate}
  \item The client needs to select a place with a power source to deploy the system.
  \item The client needs to set up a local network between the operator and the system.
\end{enumerate}
...
\begin{enumerate}
  \item What went well while writing this deliverable?\\[2mm] The scope of the work and the
        scope of the product are well defined, thanks to the detailed explanation from
        the author of the template. (There is an 80-page PDF explanation that can be found
        online.) In addition, one of the team member have experience being in a
