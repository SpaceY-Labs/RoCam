...
\textsc{Yolo26s} baseline through architecture upgrade, data engineering (COCO hard
...
\textbf{small} variant (\textsc{Yolo26s}) has ${\sim}10$M parameters and natively
detects at strides $\{8, 16, 32\}$.

\subsection{P2 Variant: Stride-4 Detection Head}

The \textsc{Yolo26s-P2} variant adds a \textbf{P2 detection head} at stride~$4$, which
...
augmentation intensity across stages, summarised in~Table~\ref{tab:augmentation}.
...
Figure~\ref{fig:map_curves} shows the mAP$_{50\text{-}95}$ progression across all
...
Figure~\ref{fig:val_loss} compares the validation box loss between the
...
Figure~\ref{fig:pr_curves} shows the precision--recall curves for the
...
Figure~\ref{fig:qualitative} provides a visual comparison of detection
...
More negatives produce a harder training problem (lower mAP$_{50\text{-}95}$) but are
...
Stage~III contributed a $+4.2\%$ improvement to mAP$_{50\text{-}95}$, compared to
...
The mAP$_{50\text{-}95}$ curve in Figure~\ref{fig:map_curves} shows the Stage~III plateau
...
detection that progresses from a vanilla \textsc{Yolo26s} baseline (mAP$_{50\text{-}95} =
  0.694$) to a final \textsc{Yolo26s-P2} model achieving mAP$_{50\text{-}95} = 0.750$ and mAP$_{50} =
