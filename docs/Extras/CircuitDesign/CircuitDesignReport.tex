...
model rocket launches using a gimbal-mounted camera. This report documents
the ``Tracking Cam Prototype 0.5 PCB,'' a development and testing platform
designed to support early software development and hardware integration for
the \progname{} project.
...
delivering 20V at 3.25A (65W). This configuration was chosen for ease of use; operators can use any USB-C PD charger or power bank
capable of 65W output.
...
The PCB dimensions are \textbf{100mm $\times$ 80mm}, matching the
footprint of the NVIDIA Jetson Orin Nano Developer Kit carrier board
(100mm x 79mm)~\citep{Nvidia_JetsonOrinNano_DevKit_Spec_2024}.
This allows the PCB to be mounted directly on top of the Jetson using
standoffs, creating a compact stack suitable for benchtop development
and testing without requiring the full outdoor gimbal assembly.

\subsection{Safety Considerations}

The onboard reset switch (SW1) controls
the enable pin of the Jetson power regulator, allowing power cycling
the Jetson and STM32. Note: See Section~\ref{sec:known-issues}
for a known issue with servo power during reset.
...
servos, and gimbal hardware. To reduce BOM cost and simplify procurement,
the \textbf{same buck converter component (TPS54308) is used for all
three voltage rails}---two servo rails and one Jetson rail---reducing
component variety and enabling volume pricing.
...
converters based on the following criteria: ~\citep{TI_TPS54308_2021}
...
used in the feedback resistor divider to set the output voltage.

\subsection{USB PD Controller: HUSB237}

The Hynetek HUSB237-AA001-DN06R USB PD sink controller was selected for
USB-C power negotiation: ~\citep{Hynetek_HUSB237_2024}
...
The STMicroelectronics STM32F405RGIx was selected for its low cost, wide availability, and compatibility with the McMaster Rocketry Team's tools and libraries. Key specifications: ~\citep{ST_STM32F405_2022}
...
The STM32F405 requires extensive decoupling for stable operation at
168MHz. Per the STM32F405 datasheet, proper decoupling is essential
for reliable operation~\citep{ST_STM32F405_2022}:
...
C100) provides the HSE (High-Speed External) clock. The X322516MLB4SI
is a 16MHz SMD crystal in a 3.2mm $\times$ 2.5mm package with 9pF
load capacitance specification~\citep{YXC_X322516MLB4SI_2024}. The
10pF load capacitors (accounting for stray capacitance and MCU pin
capacitance) ensure proper oscillation at the specified frequency.
The internal PLL multiplies the 8MHz input to the system clock
frequency (up to 168MHz)~\citep{ST_STM32F405_2022}.
...
The \textbf{reset button} (SW1, TS-1187A-B-A-B) is a 5.1mm $\times$
5.1mm SMD tactile switch rated for 12V DC, 50mA, with 100,000 cycle
operating life~\citep{XKB_TS1187A_2024}. When pressed, it pulls the
regulator EN pin high to enable the Jetson power rail.
...
Figure~\ref{fig:pcb-photo} shows the manufactured PCB before assembly.
The board features a black solder mask with white silkscreen labels
identifying connectors and test points. The component placement follows
the layout described above, with power converters in the upper left
and the STM32F405 in the lower right.
...
platform. The PCB is mounted on top of the Jetson Orin Nano Developer Kit,
with two hobby servos connected for benchtop testing of the gimbal
control firmware.
...
The following known issue has been identified in the Prototype 0.5
PCB design:

\subsection{Reset Button Does Not Cut Servo Power}

The reset button (SW1) only controls the enable pin of the Jetson
power regulator (U3). When pressed, it cuts power to the Jetson and
STM32 microcontroller, but \textbf{the servo regulators (U2) remain
powered} because they are enabled by default (EN pins tied high).

This creates a potential hazard: when the STM32 is powered off but
the servos remain powered, there is a small chance that electrical
noise or signal transients on the PWM lines could be misinterpreted
by the servos as valid commands, causing unexpected movement.
...
The Tracking Cam Prototype 0.5 PCB provides a compact development and
testing platform for the \progname{} project. The design delivers:
...
the TPS54308 0.6V reference voltage. The use of a single buck converter
component across all rails reduces BOM complexity and cost. The board
is ready for assembly and benchtop firmware development for the
\progname{} project.
