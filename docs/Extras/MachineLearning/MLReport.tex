...
\title{Multi-Stage Training Strategy for Small Rocket Detection with YOLO26s-P2 and COCO Hard Negative Mining}
\author{\authname}
\date{March 2026}
...
\begin{abstract}
  ...
  small rockets in surveillance imagery captured by the \progname{} system.
  Starting from a vanilla \textsc{Yolo26s}{} baseline
  ($\text{mAP}_{50\text{-}95} = 0.694$), we upgrade to the \textsc{Yolo26s-P2}{} architecture---which
  ...
  initially \emph{degrade} $\text{mAP}_{50\text{-}95}$ to $0.614$ ($-11.5\%$), a
  carefully designed three-stage fine-tuning protocol progressively
  recovers and surpasses the baseline, achieving a final
  $\text{mAP}_{50\text{-}95} = 0.750$ ($+8.1\%$ over the original) and
  $\text{mAP}_{50} = 0.958$.  All experiments are conducted on a
...
\textsc{Yolo26s} baseline through architecture upgrade, data engineering (COCO
hard negative mining), and a three-stage fine-tuning protocol. Our
contributions are:
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{COCO hard negative mining}: automated injection of
        4{,}000 background images from MS~COCO~\citep{lin2014coco} to
  ...
  \item \textbf{Multi-stage fine-tuning}: a three-stage protocol
        ...
        and pushes mAP$_{50\text{-}95}$ from $0.614$ to $0.750$.
...
target at small scales. To address this, we inject \textbf{pure background
  images} from MS~COCO~\citep{lin2014coco} into the training set.
...
iterative experimentation (see Section~\ref{sec:discussion}).
...
resolution of the standard finest level. This is critical for our application
where rocket targets can be as small as $10{\times}10$~pixels.

\begin{table}[h]
  \centering
  \caption{Architecture comparison: \textsc{Yolo26s} vs.\ \textsc{Yolo26s-P2}.}
  \label{tab:arch}
  \begin{tabular}{@{}lccc@{}}
    \toprule
    ...
    \textsc{Yolo26s-P2} & ${\sim}9.8$M        & $+22\%$         & $\{4, 8, 16, 32\}$         \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[
    ...
    \node[stage] (s0) at (0,0) {\textbf{Baseline}\\train63\\\textsc{Yolo26s}};
    \node[stage] (s1) at (4.2,0) {\textbf{Stage~I}\\train77\\\textsc{Yolo26s-P2}};
  ...
  \caption{Training pipeline overview.  Numbers below each stage
    indicate the best mAP$_{50\text{-}95}$ achieved. Note the initial
...
The baseline model uses the vanilla \textsc{Yolo26s} architecture trained from
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Architecture}: upgrade from \textsc{Yolo26s} to \textsc{Yolo26s-P2}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  ...
  \item $\text{lr}_0 = 0.002$ ($\frac{1}{5}$ of Stage~I),
        cosine schedule~\citep{loshchilov2016sgdr}, $\text{lrf} = 0.05$
...
($960\times544$) images for the first time, and mAP$_{50\text{-}95}$ \textbf{jumped
  from $0.614$ to $0.720$} ($+17.3\%$), reaching
mAP$_{50} = 0.955$.  This is the \textbf{single largest improvement}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
  \item Resume from \texttt{train78/best.pt} (epoch~93, mAP$_{50\text{-}95} = 0.720$)
...
\noindent\textbf{Result}: mAP$_{50\text{-}95}$ improved from $0.720$ to
$\mathbf{0.750}$ ($+4.2\%$), with mAP$_{50} = 0.958$,
...
augmentation intensity across stages, summarised
in~Table~\ref{tab:augmentation}.
...
compatibility~\citep{buslaev2020albumentations}. The pipeline includes:
...
Table~\ref{tab:results} presents the best metrics for each training stage.
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}llcccccc@{}}
    ...
    \textbf{Precision} & \textbf{Recall} & \textbf{mAP$_{50}$} &
    \textbf{mAP$_{50\text{-}95}$}                                                                                                                      \\
    \midrule
    Baseline           & train63         & \textsc{Yolo26s}    & 419               & 0.945          & 0.897          & 0.948          & 0.694          \\
    \midrule
    Stage~I (v1)       & train76         & \textsc{Yolo26s-P2} & 335               & 0.918          & 0.886          & 0.935          & 0.652          \\
    Stage~I (v2)       & train77         & \textsc{Yolo26s-P2} & 267               & 0.917          & 0.857          & 0.920          & 0.614          \\
    \midrule
    Stage~II           & train78         & \textsc{Yolo26s-P2} & 93                & 0.955          & 0.912          & 0.955          & 0.720          \\
    \midrule
    Stage~III          & train81         & \textsc{Yolo26s-P2} & 52                & \textbf{0.961} & \textbf{0.915} & \textbf{0.958} & \textbf{0.750} \\
    \bottomrule
  \end{tabular}
\end{table}
...
Figure~\ref{fig:map_curves} shows the mAP$_{50\text{-}95}$ progression across
all stages, clearly illustrating the initial degradation and subsequent
recovery.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addlegendentry{Stage~I (train77, \textsc{Yolo26s-P2} + COCO neg)}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{mAP$_{50\text{-}95}$ learning curves across all stages.  The dashed
...
Figure~\ref{fig:val_loss} compares the validation box loss between the baseline
and Stage~I, revealing the convergence pattern.
...
\begin{figure}[H]
  ...
  \caption{Validation box loss comparison.  The baseline (train63,
    \textsc{Yolo26s}) continues to decrease throughout 420 epochs, while
    Stage~I (train77, \textsc{Yolo26s-P2}) \emph{stagnates} around epoch~250 and
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{mAP$_{50\text{-}95}$ progression across training stages.  The initial
...
Figure~\ref{fig:pr_curves} shows the precision--recall curves for the baseline
and the final model, generated by Ultralytics.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Final model (train81): mAP$_{50} = 0.958$}
  \end{subfigure}
...
Figure~\ref{fig:cm} presents the normalised confusion matrices, showing the
...
Figure~\ref{fig:qualitative} provides a visual comparison of detection quality
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{COCO negative dilution}: Adding 4{,}000 background
        ...
        mAP$_{50\text{-}95}$ on the validation set that contains no COCO images.

  \item \textbf{Mosaic starvation}: Mosaic augmentation~\citep{bochkovskiy2020yolov4}
...
More negatives produce a harder training problem (lower mAP$_{50\text{-}95}$)
but are expected to improve \emph{deployment} performance by suppressing false
...
Stage~III contributed a $+4.2\%$ improvement to mAP$_{50\text{-}95}$, compared
to Stage~II's $+17.3\%$. This is expected: as the learning rate decreases
($0.0003$ vs.\ $0.002$), the model makes increasingly small parameter updates.
The mAP$_{50\text{-}95}$ curve in Figure~\ref{fig:map_curves} shows the
Stage~III plateau beginning around epoch~35, suggesting the model has
...
detection that progresses from a vanilla \textsc{Yolo26s} baseline
(mAP$_{50\text{-}95} = 0.694$) to a final \textsc{Yolo26s-P2} model achieving
mAP$_{50\text{-}95} = 0.750$ and mAP$_{50} = 0.958$. The key insights are:
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lcccc@{}}
    ...
    \midrule
    \textbf{Best mAP$_{50\text{-}95}$} & \textbf{0.694}    & \textbf{0.614}   & \textbf{0.720}    & \textbf{0.750}     \\
    \bottomrule
  \end{tabular}
\end{table}
...
Figure~\ref{fig:ultralytics_plots} shows the built-in Ultralytics training
