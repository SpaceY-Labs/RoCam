...
\begin{tabularx}{\textwidth}{p{3cm}p{2cm}X}
  ...
  Feb. 4, 2026        & Rev 1         & Added gimbal\_info (0x00); renumbered commands; renamed Move$\to$Set Deg, Measure$\to$Get Deg; Get GPS Data moved to 0x07 \\
  \bottomrule
\end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{ccX}
    ...
    \midrule
    \texttt{0x00}       & Gimbal Info      & Get tilt, pan, and focal length ranges              \\
    \texttt{0x01}       & Set ARM LED      & Turn the ARM indicator LED on or off                \\
    \texttt{0x02}       & Set Status LED   & Turn the Status indicator LED on or off             \\
    \texttt{0x03}       & Set Deg          & Move the gimbal to a specified tilt and pan angle   \\
    \texttt{0x04}       & Get Deg          & Read the current tilt and pan angles                \\
    ...
    \texttt{0x07}       & Get GPS Data     & Read the current GPS coordinates and timestamp      \\
    \bottomrule
  \end{tabularx}
  \caption{Command Summary}
\end{table}

\subsection{Gimbal Info (Command ID: \texttt{0x00})}

\subsubsection{Purpose}
This command requests capability information from the gimbal. The gimbal
responds with the valid ranges for tilt (degrees), pan (degrees), and focal
length (millimeters). Each range is given as a minimum and maximum value (two
float32 values per range).
...
\begin{table}[H]
  ...
  \begin{tabular}{cccl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Gimbal Info Request (2 bytes total)}
\end{table}

Since there is no payload, the CRC is calculated over just the Command ID byte
(\texttt{0x00}), which results in \texttt{0x00}.
...
\begin{table}[H]
  ...
  \begin{tabular}{cccl}
    ...
    \textbf{Byte} & \textbf{Size} & \textbf{Value} & \textbf{Description}            \\
    \midrule
    0..3          & 4             & (float32 LE)   & Tilt range minimum (degrees)    \\
    4..7          & 4             & (float32 LE)   & Tilt range maximum (degrees)    \\
    8..11         & 4             & (float32 LE)   & Pan range minimum (degrees)     \\
    12..15        & 4             & (float32 LE)   & Pan range maximum (degrees)     \\
    16..19        & 4             & (float32 LE)   & Focal length range minimum (mm) \\
    20..23        & 4             & (float32 LE)   & Focal length range maximum (mm) \\
    24            & 1             & (calculated)   & CRC-8 over bytes [0..23]        \\
    \bottomrule
  \end{tabular}
  \caption{Gimbal Info Response (25 bytes total)}
\end{table}

\subsubsection{Example}

Request to read gimbal info:
\begin{itemize}
  \item Bytes to CRC: \texttt{0x00}
  \item CRC result: \texttt{0x00}
  \item Complete request: \texttt{0x00 0x00}
\end{itemize}

Example response (tilt range $-30$ to $30$°, pan range $-180$ to $180$°, focal
length 10 to 100 mm):
\begin{itemize}
  \item Tilt min ($-30$ as float32 LE): \texttt{0x00 0x00 0xF0 0xC1}; tilt max (30):
        \texttt{0x00 0x00 0xF0 0x41}
  \item Pan min ($-180$ as float32 LE): \texttt{0x00 0x00 0x34 0xC3}; pan max (180):
        \texttt{0x00 0x00 0x34 0x43}
  \item Focal min (10 as float32 LE): \texttt{0x00 0x00 0x20 0x41}; focal max (100):
        \texttt{0x00 0x00 0xC8 0x42}
  \item Data bytes: \texttt{0x00 0x00 0xF0 0xC1 0x00 0x00 0xF0 0x41 0x00 0x00 0x34 0xC3
          0x00 0x00 0x34 0x43 0x00 0x00 0x20 0x41 0x00 0x00 0xC8 0x42}
  \item CRC over data: \texttt{0x93}
  \item Complete response: \texttt{0x00 0x00 0xF0 0xC1 0x00 0x00 0xF0 0x41 0x00 0x00
          0x34 0xC3 0x00 0x00 0x34 0x43 0x00 0x00 0x20 0x41 0x00 0x00 0xC8 0x42 0x93}
\end{itemize}

\subsection{Set ARM LED (Command ID: \texttt{0x01})}
...
\begin{itemize}
  ...
  \item Bytes to CRC: \texttt{0x01 0x01} (Command ID + payload)
  \item CRC result: \texttt{0x12}
  \item Complete request: \texttt{0x12 0x01 0x01}
  \item Expected response: \texttt{0x00}
\end{itemize}

\subsection{Set Status LED (Command ID: \texttt{0x02})}
...
\begin{itemize}
  ...
  \item Bytes to CRC: \texttt{0x02 0x00} (Command ID + payload)
  \item CRC result: \texttt{0x2A}
  \item Complete request: \texttt{0x2A 0x02 0x00}
  \item Expected response: \texttt{0x00}
\end{itemize}

\subsection{Set Deg (Command ID: \texttt{0x03})}
...
\begin{table}[H]
  ...
  \begin{tabular}{cccl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Set Deg Request (10 bytes total)}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{cccl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Set Deg Response (1 byte total)}
\end{table}
...
\begin{itemize}
  ...
  \item Bytes to CRC: \texttt{0x03 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00}
  \item CRC result: \texttt{0x8B}
  \item Complete request: \texttt{0x8B 0x03 0x00 0x00 0x00 0x00 0x00 0x00 0x00 0x00}
  \item Expected response: \texttt{0x00}
\end{itemize}

\subsection{Get Deg (Command ID: \texttt{0x04})}
...
\begin{table}[H]
  ...
  \begin{tabular}{cccl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Get Deg Request (2 bytes total)}
\end{table}

Since there is no payload, the CRC is calculated over just the Command ID byte
(\texttt{0x04}), which results in \texttt{0x1C}.
...
\begin{table}[H]
  ...
  \begin{tabular}{cccl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Get Deg Response (9 bytes total)}
\end{table}
...
\begin{itemize}
  ...
  \item CRC result: \texttt{0x1C}
  \item Complete request: \texttt{0x1C 0x04}
\end{itemize}
...
\begin{itemize}
  ...
  \item CRC over data: \texttt{0x58}
  \item Complete response: \texttt{0x00 0x00 0x48 0x41 0x00 0x00 0x50 0x40 0x58}
\end{itemize}
...
\begin{itemize}
  ...
  \item CRC result: \texttt{0xD7}
  \item Complete request: \texttt{0xD7 0x05 0x00 0x00 0x48 0x42}
  \item Expected response: \texttt{0x00}
\end{itemize}
...
Since there is no payload, the CRC is calculated over just the Command ID byte
(\texttt{0x06}), which results in \texttt{0x12}.
...
\begin{itemize}
  ...
  \item CRC result: \texttt{0x12}
  \item Complete request: \texttt{0x12 0x06}
\end{itemize}
...
\begin{itemize}
  ...
  \item CRC over data: \texttt{0x3A}
  \item Complete response: \texttt{0x00 0x00 0x48 0x42 0x3A}
\end{itemize}

\subsection{Get GPS Data (Command ID: \texttt{0x07})}

\subsubsection{Purpose}
This command requests the current GPS data from the gimbal. The gimbal responds
with its current longitude, latitude, and Unix timestamp in milliseconds. If
coordinates are not available, longitude and latitude are returned as NaN. If
the timestamp is not available, it is returned as zero. GPS typically acquires
time before coordinates.
...
Since there is no payload, the CRC is calculated over just the Command ID byte
(\texttt{0x07}), which results in \texttt{0x15}.
...
\begin{itemize}
  \item Bytes to CRC: \texttt{0x07}
  \item CRC result: \texttt{0x15}
  \item Complete request: \texttt{0x15 0x07}
\end{itemize}

Example response with full GPS lock (longitude = -79.9167°, latitude =
43.2567°, timestamp = 1705123456789 ms):
\begin{itemize}
  \item Longitude (-79.9167 as float64 LE): \texttt{0x91 0x0F 0x7A 0x36 0xAB 0xFA 0x53
          0xC0}
  \item Latitude (43.2567 as float64 LE): \texttt{0x0D 0x71 0xAC 0x8B 0xDB 0xA0 0x45
          0x40}
  \item Timestamp (1705123456789 as u64 LE): \texttt{0x15 0x27 0x47 0x01 0x8D 0x01 0x00
          0x00}
  \item Data bytes: \texttt{0x91 0x0F 0x7A 0x36 0xAB 0xFA 0x53 0xC0 0x0D 0x71 0xAC 0x8B
          0xDB 0xA0 0x45 0x40 0x15 0x27 0x47 0x01 0x8D 0x01 0x00 0x00}
  \item CRC over data: \texttt{0x97}
  \item Complete response: (data bytes + \texttt{0x97})
\end{itemize}
...
\begin{itemize}
  ...
  \item Data bytes: \texttt{0x00 0x00 0x00 0x00 0x00 0x00 0xF8 0x7F 0x00 0x00 0x00 0x00
          0x00 0x00 0xF8 0x7F 0x15 0x27 0x47 0x01 0x8D 0x01 0x00 0x00}
  \item CRC over data: \texttt{0x37}
  \item Complete response: (data bytes + \texttt{0x37})
\end{itemize}
...
\begin{itemize}
  ...
  \item CRC over data: \texttt{0x82}
  \item Complete response: (data bytes + \texttt{0x82})
\end{itemize}
