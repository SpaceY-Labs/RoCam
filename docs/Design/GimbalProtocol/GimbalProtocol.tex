...
This command requests capability information from the gimbal. The gimbal responds
with the valid ranges for tilt (degrees), pan (degrees), and focal length
(millimeters). Each range is given as a minimum and maximum value (two float32
values per range).
...
length 10 to 100 mm):
\begin{itemize}
  \item Tilt min ($-30$ as float32 LE): \texttt{0x00 0x00 0xF0 0xC1}; tilt max (30): \texttt{0x00 0x00 0xF0 0x41}
  \item Pan min ($-180$ as float32 LE): \texttt{0x00 0x00 0x34 0xC3}; pan max (180): \texttt{0x00 0x00 0x34 0x43}
  \item Focal min (10 as float32 LE): \texttt{0x00 0x00 0x20 0x41}; focal max (100): \texttt{0x00 0x00 0xC8 0x42}
  ...
  \item Complete response: \texttt{0x00 0x00 0xF0 0xC1 0x00 0x00 0xF0 0x41 0x00 0x00 0x34 0xC3
          0x00 0x00 0x34 0x43 0x00 0x00 0x20 0x41 0x00 0x00 0xC8 0x42 0x93}
\end{itemize}
