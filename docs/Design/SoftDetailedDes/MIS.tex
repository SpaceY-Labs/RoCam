...
\begin{enumerate}
  ...
  \item Give a brief overview of other design solutions you considered. What are the
    ...
    \item Reflection on implementing the other group's module:\\
        ...
        When we started, it was hard to tell the exact scope of M9 because many UI actions
        depend on backend APIs. We checked their documents, but we only found limited UI
        details and no clear API specification or existing code to connect to. Because of
        time constraints, we implemented a reasonable UI based on the requirements we had and
        made the API calls a clean placeholder so it can be connected later. We also opened a
        PR and left notes on what endpoints and behaviours would be needed for full
        integration. This reminded us that clear module boundaries and early API contracts
