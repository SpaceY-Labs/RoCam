...
\begin{itemize}
  ...
  \item output: (tilt\_range\_deg: (f32, f32), pan\_range\_deg: (f32, f32), focal\_length\_range\_mm: (f32, f32))
...
\begin{itemize}
  \item transition: Retrieves the tilt and pan angle measurements from the gimbal
        ...
        \href{https://github.com/SpaceY-Labs/RoCam/blob/main/docs/Design/GimbalProtocol/GimbalProtocol.pdf}{Gimbal
          Protocol} doc for details).
  \item output: (tilt\_deg: f32, pan\_deg: f32). If coordinates are not available, returns (NaN, NaN).
  \item exception: GimbalError
\end{itemize}
...
\begin{itemize}
  \item transition: Retrieves GPS data from the gimbal through the Serial Abstraction
        ...
        \href{https://github.com/SpaceY-Labs/RoCam/blob/main/docs/Design/GimbalProtocol/GimbalProtocol.pdf}{Gimbal
          Protocol} doc for details).
  \item output: (longitude: f64, latitude: f64, timestamp\_ms: u64). If coordinates are not available, longitude and latitude are NaN. If timestamp is not available, timestamp\_ms is 0.
  \item exception: GimbalError
\end{itemize}
...
\begin{itemize}
  \item transition: Return \texttt{SystemStatus} with the following fields:
        \begin{itemize}
          ...
          \item longitude: First element of \texttt{gimbal.get\_gps\_data()} (longitude); if NaN, null.
          \item latitude: Second element of \texttt{gimbal.get\_gps\_data()} (latitude); if NaN, null.
          ...
          \item tracking\_state:
                \begin{itemize}
                  ...
                  \item If \texttt{is\_armed} is true, and \texttt{cv\_data.bbox} is null, return
                        'armed'.
                \end{itemize}
          \item longitude: First element of \texttt{gimbal.get\_gps\_data()} (longitude); if NaN, 0.0.
          \item latitude: Second element of \texttt{gimbal.get\_gps\_data()} (latitude); if NaN, 0.0.
        \end{itemize}
        ...
        \begin{itemize}
          \item Loop every 5 seconds.
                \begin{itemize}
                  ...
                  \item If timestamp\_ms (third element) is not 0, set the system time to the timestamp and break
                        the loop.
                \end{itemize}.
        \end{itemize}
  \item output: None
