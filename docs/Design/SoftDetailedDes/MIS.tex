...
\begin{enumerate}
  ...
  \item  The other group's capstone vision which is a AI voice powered computer for
        people with disabilities. Our team is facinated by their vision because this is
        our top backup project. The module our team was assigned to implement is the
        entire UI of the AI voice pwoered computer. Our team was confused at first
        because the entire UI is not traditionally considered to be a module. We begin
        by searching through relevant documentation about the UI requirements and API
        specification or code. Unfortunately, we only found minimal documentation on UI
        and no API documentation or code implementation. Despite the lack of
        information, our team decided to move forward and implement a UI to the best of
        our ability based on the existing requirement documents. After the feature is
        done, we opened a PR and left a comment outlined the lack of documentation
        during our implementation. By the time writing this reflection, the other group
        is inresponsive to our comments. As a reflection, we understood the importance
        of documentation and definition when working on a collaborative project since
        it is difficult to work on a project if the feature is not properly specified
        and no effective communication is done. We also learned that it is important to
        consider the size of each module to properly distribute the workloads.
\end{enumerate}
