...
\begin{tabularx}{\textwidth}{p{3cm}p{2cm}X}
  ...
  Jan. 19, 2026       & Rev 0         & Team Work                \\
  Jan. 21, 2026       & Rev 0         & Team Review              \\
  Apr. 5, 2026        & Rev 1         & Address TA, peer, and instructor feedback; fix typos,
                                        remove template comments; improve descriptions           \\
  \bottomrule
\end{tabularx}
...
\begin{center}
  \begin{tabular}{p{4cm} p{3cm} p{8cm}}
    ...
    system\_power\_w            & float                             & System power draw in watts                                      \\
...
%7
\section{MIS of CV Process Module (\mref{mCV})}
...
%8
\section{MIS of Live Video Process Module (\mref{mVideo})}
...
%9
\section{MIS of Transcode Process Module (\mref{mTranscode})}
...
%10
\section{MIS of Video Stream Abstraction Module (\mref{mStream})}
...
%11
\section{MIS of CV Process Management Module (\mref{mCVManagement})}
...
%12
\section{MIS of Live Video Process Management Module (\mref{mVideoManagement})}
...
%13
\section{MIS of Transcode Process Management Module (\mref{mTranscodeManagement})}
...
%14
\section{MIS of Tracking Module (\mref{mTrack})}
...
\begin{itemize}
  \item transition:
        \begin{enumerate}
          \item Get the current gimbal state from \texttt{gimbal.get\_deg()} and
                \texttt{gimbal.get\_focal\_length\_mm()}.
          ...
          \item Set the desired gimbal state using \texttt{gimbal.set\_deg()} and
                \texttt{gimbal.set\_focal\_length\_mm()}.
        \end{enumerate}
  \item output: None
...
%15
\section{MIS of Gimbal Abstraction Module (\mref{mGimbal})}
...
\begin{center}
  \begin{tabular}{p{3cm} p{4cm} p{4cm} p{3.5cm}}
    ...
    \hline
    gimbal\_info           & -                                     & tilt\_range\_deg: (f32, f32), pan\_range\_deg: (f32, f32), focal\_length\_range\_mm: (f32, f32) & GimbalError         \\
    \hline
    set\_arm\_led          & enabled: bool                         & -                                                                                               & GimbalError         \\
    \hline
    set\_status\_led       & enabled: bool                         & -                                                                                               & GimbalError         \\
    \hline
    set\_deg               & tilt\_deg: f32 \newline pan\_deg: f32 & -                                                                                               & GimbalError         \\
    \hline
    get\_deg               & -                                     & tilt\_deg: f32, pan\_deg: f32                                                                   & GimbalError         \\
    \hline
    set\_focal\_length\_mm & focal\_length\_mm: f32                & -                                                                                               & GimbalError         \\
    \hline
    get\_focal\_length\_mm & -                                     & focal\_length\_mm: f32                                                                          & GimbalError         \\
    \hline
    get\_gps\_data         & -                                     & longitude: f64, latitude: f64, timestamp\_ms: u64                                               & GimbalError         \\
    \hline
  \end{tabular}
\end{center}

\noindent For \texttt{get\_deg}: if coordinates are not available, returns (NaN, NaN). For \texttt{get\_gps\_data}: if coordinates are not available, longitude and latitude are NaN; if timestamp is not available, timestamp\_ms is 0.
...
\noindent gimbal\_info():
\begin{itemize}
  \item transition: Retrieves tilt, pan, and focal length ranges from the gimbal
  ...
  \item output: (tilt\_range\_deg: (f32, f32), pan\_range\_deg: (f32, f32),
        focal\_length\_range\_mm: (f32, f32))
  \item exception: GimbalError
\end{itemize}

\noindent set\_arm\_led(enabled: bool):
\begin{itemize}
...
\noindent set\_status\_led(enabled: bool):
\begin{itemize}
...
\noindent set\_deg(tilt\_deg: f32, pan\_deg: f32):
\begin{itemize}
...
\begin{itemize}
  \item transition: Retrieves the tilt and pan angle measurements from the gimbal
        ...
        \href{https://github.com/SpaceY-Labs/RoCam/blob/main/docs/Design/GimbalProtocol/GimbalProtocol.pdf}{Gimbal
          Protocol} doc for details).
  \item output: (tilt\_deg: f32, pan\_deg: f32). If coordinates are not available,
        returns (NaN, NaN).
  \item exception: GimbalError
\end{itemize}

\noindent set\_focal\_length\_mm(focal\_length\_mm: f32):
\begin{itemize}
...
\noindent get\_focal\_length\_mm():
\begin{itemize}
...
\begin{itemize}
  \item transition: Retrieves GPS data from the gimbal through the Serial Abstraction
        ...
        \href{https://github.com/SpaceY-Labs/RoCam/blob/main/docs/Design/GimbalProtocol/GimbalProtocol.pdf}{Gimbal
          Protocol} doc for details).
  \item output: (longitude: f64, latitude: f64, timestamp\_ms: u64). If coordinates are
        not available, longitude and latitude are NaN. If timestamp is not available,
        timestamp\_ms is 0.
  \item exception: GimbalError
\end{itemize}
...
% 16
%16
\section{MIS of Serial Abstraction Module (\mref{mSerial})}
...
% 17
%17
\section{MIS of System Status Module (\mref{mStatus})}
...
%18
\section{MIS of Recording Database Module (\mref{mRecord})}
...
%19
\section{MIS of State Management Module (\mref{mState})}
...
\begin{itemize}
  \item transition:
        \begin{itemize}
          ...
          \item Call \texttt{gimbal.set\_arm\_led(true)}.
        \end{itemize}
  \item output: None
  \item exception: None
\end{itemize}
...
\begin{itemize}
  \item transition:
        \begin{itemize}
          \item Set \texttt{is\_armed} to false.
          \item Call \texttt{gimbal.set\_arm\_led(false)}.
        \end{itemize}
  \item output: None
  \item exception: None
\end{itemize}
...
\begin{itemize}
  \item transition: Return \texttt{SystemStatus} with the following fields:
        \begin{itemize}
          ...
          \item gimbal\_tilt\_deg: From \texttt{gimbal.get\_deg()}.
          \item gimbal\_pan\_deg: From \texttt{gimbal.get\_deg()}.
          \item gimbal\_focal\_length\_mm: From \texttt{gimbal.get\_focal\_length\_mm()}.
          ...
          \item longitude: First element of \texttt{gimbal.get\_gps\_data()} (longitude); if
                NaN, null.
          \item latitude: Second element of \texttt{gimbal.get\_gps\_data()} (latitude); if
                NaN, null.
          ...
          \item system\_power\_w: From \texttt{system\_status.get\_system\_power\_w()}.
...
\begin{itemize}
  \item transition:
        \begin{itemize}
          \item Get current gimbal angle from \texttt{gimbal.get\_deg()}.
          \item Calculate the new gimbal angle based on the given direction.
                ...
                \end{itemize}
          \item Call \texttt{gimbal.set\_deg(new\_tilt, new\_pan)}.
        \end{itemize}
  \item output: None
  \item exception: None
\end{itemize}
...
\begin{itemize}
  \item transition: Call \texttt{gimbal.set\_deg(tilt, pan)}
  \item output: None
  \item exception: None
\end{itemize}
        ...
        \begin{itemize}
          \item Loop every 5 seconds.
                \begin{itemize}
                  ...
                  \item If timestamp\_ms (third element) is not 0, set the system time to the timestamp
                        and break the loop.
                \end{itemize}.
        \end{itemize}
  \item output: None
...
%20
\section{MIS of API Gateway Module (\mref{mAPI})}
...
%21
\section{MIS of Preview Module (\mref{mPreview})}
...
%22
\section{MIS of Manual Control Module (\mref{mManual})}
...
%23
\section{MIS of Recording Management Module (\mref{mRecordMng})}
...
\begin{enumerate}
  ...
  \item Reflection on implementing the other group's module:

        The other group's capstone vision is an AI voice-powered computer for people
        with disabilities. We were assigned to implement the \textit{User Interface
          Panel (M9)} module. Their idea was open-ended in a good way, which gave us a
        lot of space to code and translate their concept into a concrete UI.

        \textbf{what went well:}
        \begin{itemize}
          \item The vision and requirements left room for design decisions, so we could propose
                a clean, user-friendly UI and deliver working screens quickly.
          \item We kept the UI modular (components + typed state), so it is easy to extend and
                connect to real backend endpoints later.
          \item We documented assumptions and integration needs directly in the PR, making
                handoff clearer.
        \end{itemize}

        \textbf{Challenges we faced:}
        \begin{itemize}
          \item The scope of M9 was hard to define because many UI actions depend on backend
                APIs.
          \item We found limited UI documentation and no clear API specification or existing
                code to integrate with, so we had to use placeholders and mock behaviour.
          \item Without an agreed API contract, some UI flows (e.g., confirmations, error
                states, progress) could only be implemented as best-effort defaults.
        \end{itemize}

        \textbf{How the other team can improve in the future:}
        \begin{itemize}
          \item Publish a minimal API contract early (endpoints, request/response examples,
                error codes), even if the backend is not finished.
          \item Define UI module boundaries explicitly (what is in M9 vs. backend vs.
                voice/agent logic), and include acceptance criteria for key user flows.
          \item Add a short ``integration checklist'' (auth, data shape, loading/error states)
                so another team can implement UI without guessing.
        \end{itemize}

        \textbf{What we learned:}
        \begin{itemize}
          \item Clear module definition and early interface documentation are essential for
                parallel work.
          \item When dependencies are missing, a good fallback is to build a UI skeleton with
                clean placeholders, typed interfaces, and obvious extension points.
          \item Writing down assumptions and integration needs in the PR helps reduce ambiguity
                and makes future integration faster.
        \end{itemize}

        Below are the frontend files we implemented for the other team (PRs:
        \url{https://github.com/gkpodder/Capstone/pulls}):

        \begin{itemize}
          \item \texttt{src/frontend/my-electron-app/}
                \begin{itemize}
                  \item \textbf{Core app (\texttt{src/})}
                        \begin{itemize}
                          \item \texttt{App.tsx}, \texttt{main.tsx}
                          \item \texttt{types.ts}, \texttt{constants.ts}
                          \item \texttt{App.css}, \texttt{index.css}
                          \item \texttt{assets/react.svg}
                        \end{itemize}

                  \item \textbf{API mock (\texttt{src/api/})}
                        \begin{itemize}
                          \item \texttt{API.js}, \texttt{API.d.ts}
                        \end{itemize}

                  \item \textbf{UI components (\texttt{src/components/})}
                        \begin{itemize}
                          \item \texttt{TopBar.tsx}, \texttt{CommandPanel.tsx}, \texttt{TranscriptPanel.tsx}
                          \item \texttt{ApprovalCard.tsx}, \texttt{ActionRow.tsx}
                        \end{itemize}

                  \item \textbf{Project config}
                        \begin{itemize}
                          \item \texttt{index.html}, \texttt{package.json}, \texttt{package-lock.json}
                          \item \texttt{vite.config.ts}, \texttt{eslint.config.js}
                          \item \texttt{tsconfig.json}, \texttt{tsconfig.app.json}, \texttt{tsconfig.node.json}
                        \end{itemize}
                \end{itemize}
        \end{itemize}
