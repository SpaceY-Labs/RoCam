...
\begin{table}[hp]
  ...
  \begin{tabularx}{\textwidth}{llX}
    ...
    \midrule
    Oct. 5, 2025  & Shike Chen            & Initial draft                                                                                                      \\
    Oct. 19, 2025 & Shike Chen            & Detailed draft                                                                                                     \\
    Apr. 5, 2026  & All team members      & Rev 1: Address feedback, fix typos, remove template comments, add traceability to SRS, improve safety requirements \\
    \bottomrule
  \end{tabularx}
\end{table}
...
Such hazard analysis will not only be used to identify potential hazards caused
by the system, but also to minimize risks related to unforeseen cases. The
purpose of this hazard analysis is to properly address the potential harm and
risks caused by the product to the user and the external environment.
...
The camera gimbal used in this project is an existing camera gimbal designed by
...
The Jetson Orin Nano is a small, lightweight AI computer designed by NVIDIA. It
is used to deploy our computer vision module and communicate directly with the
motion control module.
\subsubsection{Camera}
The camera is an off-the-shelf camera that is compatible with the Jetson Orin
...
will be torch and pandas. It will be accelerated with TensorRT on the Jetson to
...
The system will not operate in any extreme environment (e.g., extreme
temperature, acidic environments, etc.). The system will be operating within 10
to 25 degrees Celsius. The system will operate with a stable power supply.
\subsection{Assumptions about the User}
Anyone above 14 years old who uses electronic devices and is familiar with
software interfaces on a daily basis for over 2 years, with good intent.
\subsection{Assumptions about Off-the-Shelf Hardware Components}
    ...
    \begin{tblr}{
      ...
      \begin{enumerate}
        \item \begin{enumerate}
                ...
                \item failed in communication between CV module and the motion control module
        ...
        \item \begin{enumerate}
                ...
                \item rocket angular velocity relative to the gimbal exceeds the design limit
              \end{enumerate}
        \item \begin{enumerate}
                \item improper instruction from the motion control module
      ...
      \begin{enumerate}
        ...
        \item \begin{enumerate}
                ...
                \item design motion control module and CV module speed to have self-correcting (SR3)
                      mechanism
                \item design multiple tracking algorithms to avoid losing the rocket
...
\begin{enumerate}
  \item \textbf{SR1} (Angular Velocity Limit): The system shall be placed at a distance
        such that the expected angular velocity of the rocket relative to the camera
        does not exceed the design limit of the gimbal. The system shall enforce
        software-defined angular velocity and angle limits to prevent mechanical
        damage.\\
        \textit{Traced to SRS:} SCR-1 (manual override to idle), RFR-2 (safe state on
        unrecoverable error), IR-1 (input validation for gimbal commands).

  \item \textbf{SR2} (Personal Protective Equipment): All operators must wear safety
        goggles when operating the system in proximity to the gimbal hardware.\\
        \textit{Traced to SRS:} SCR-3 (remote operation over local network reduces need
        for physical proximity during flight).

  \item \textbf{SR3} (Self-Correcting Tracking): The system shall have a self-correcting
        mechanism to ensure the camera can track the rocket smoothly in real time.
        If the target is temporarily lost, the system shall attempt re-acquisition
        before declaring the target lost and returning to idle state.\\
        \textit{Traced to SRS:} FR-7 (keep rocket within frame), RFR-4
        (autonomous re-acquisition before declaring loss), RFR-5 (high success
        rate of tracking).

  \item \textbf{SR4} (Emergency Stop): The system shall provide an emergency stop
        mechanism that immediately ceases all automated gimbal movement and
        transitions the system to a safe idle state within 0.5 seconds.\\
        \textit{Traced to SRS:} SCR-1 (manual exit from armed/tracking mode),
        RFR-2 (halt on unrecoverable error), EZ-4 (help user avoid mistakes).
\end{enumerate}

\section{Traceability Between Hazard Analysis and SRS}

Table~\ref{tab:ha-srs-trace} maps each safety requirement identified in this
hazard analysis to the corresponding requirements in the Software Requirements
Specification (SRS). This ensures bi-directional traceability: every safety
concern is addressed by at least one verifiable SRS requirement, and key SRS
safety-related requirements trace back to an identified hazard.

\begin{table}[H]
  \caption{Traceability Matrix: HA Safety Requirements $\leftrightarrow$ SRS Requirements}\label{tab:ha-srs-trace}
  \begin{tabularx}{\textwidth}{l l Y}
    \toprule
    \textbf{HA Safety Req.}        & \textbf{SRS Requirement(s)} & \textbf{Rationale}                                                                                                       \\
    \midrule
    SR1 (Angular Velocity Limit)   & SCR-1, RFR-2, IR-1, INT-2   & Gimbal velocity/angle limits enforced in software; manual override and safe-state transitions prevent mechanical damage. \\
    SR2 (PPE)                      & SCR-3                       & Remote operation reduces physical proximity risk; PPE required when near hardware.                                       \\
    SR3 (Self-Correcting Tracking) & FR-7, RFR-4, RFR-5          & Autonomous tracking with re-acquisition ensures robust rocket following and graceful degradation on target loss.         \\
    SR4 (Emergency Stop)           & SCR-1, RFR-2, EZ-4          & Manual exit to idle within 0.5\,s; safe-state halt on unrecoverable error; confirmation prompts for dangerous commands.  \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{C{1.2cm} L{4.2cm} Y Y Y}
    ...
    All SRs: SR1--SR4 complete and traced.                                                                                        &
...
\begin{enumerate}
  ...
  \item M4 (End Sprint 8): Safety package complete (FMEA v2, SR1--SR4 verified,
        documentation).
\end{enumerate}
...
\begin{itemize}
  \item FMEA \(\rightarrow\) SR mapping: “Gimbal snaps/overshoots” \(\rightarrow\) SR1;
        “Tracking lost” \(\rightarrow\) SR3; “Emergency halt needed” \(\rightarrow\)
        SR4; “UI unavailable/unclear” \(\rightarrow\) UI alarms, SOP, tests.
...
\begin{itemize}
  ...
  \item \textbf{SR4}: Emergency stop reaches idle state within 0.5\,s from any operational state; evidence: timed tests + command logs.
\end{itemize}
