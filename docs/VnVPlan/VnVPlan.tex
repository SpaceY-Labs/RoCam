...
possible. \textbf{The only testing performed is unit testing;} integration,
system, and field testing are out of scope. Verification relies on structured
internal reviews and checklists, plus unit tests, to confirm functional
behavior and safety assumptions. Validation is limited to a planned review with
the McMaster Rocketry Team (stakeholders) to collect feedback; no task-based
...
components, and all testing beyond unit tests) are noted to keep the plan
feasible. The document concludes with traceability tables, placeholders for
unit-level details after design finalization, and appendices for parameters and
optional surveys---forming a clear, practical roadmap for the evidence of
quality that can be gathered under these constraints.
...
unit testing and internal review to confirm that functional and performance
criteria are addressed where possible, with improvements tracked through GitHub
Issues. Validation focuses on a planned review with the McMaster Rocketry Team
(stakeholders) to collect feedback; no field testing or supervisor validation
is performed. These qualities—correctness, performance, reliability, and
traceability—are fundamental to the success of the V\&V process, ensuring that
the RoCam system is both technically sound and operationally dependable.
Certain objectives, such as independent verification of third-party hardware
and extended usability testing, are excluded due to scope and resource
limitations, with reliance placed on manufacturer and stakeholder validation.
...
reviews and unit testing only, as well as the validation strategy through a
planned stakeholder review with the McMaster Rocketry Team (no field testing).
Its purpose is to provide a clear, organized framework ensuring that all
aspects of RoCam are systematically tested, reviewed, and proven reliable
before final deployment. \wss{Introduce this section. You can provide a roadmap
...
He will be the one to review the system design and implementation and the
communication bridge between the team and the stakeholders.
...
integration tests, or supervisor validation are performed; the only testing
conducted is unit testing.
...
Due to resource and contact limitations, \textbf{only unit testing} is
performed. Integration testing, system testing, and field testing are out of
scope.
...
\noindent\textit{Out of scope:} Integration testing, system testing, and
field testing are not performed due to the constraints noted in this plan.
...
\textbf{Scope note:} Due to resource and contact limitations, the only testing
actually performed in this project is \textbf{unit testing}. The system tests
and field tests described in this section are documented for reference and
traceability but are \textbf{out of scope} and will not be executed. Evidence
of correctness will come from unit tests and from the planned stakeholder
review with the McMaster Rocketry Team.
...
(Appendix 6.2). Under the current plan, no field testing is performed; only
unit testing and a planned stakeholder review with the rocketry team are
conducted.
...
Field testing and user surveys are out of scope; only unit testing and the
stakeholder review are performed.

\subsubsection*{test-user-survey: User Survey (Out of Scope)}
\begin{itemize}
  \item \textbf{Type:} Static (out of scope)
  \item \textbf{Initial State:} N/A
  \item \textbf{Input/Condition:} N/A
  \item \textbf{Output/Result:} Completed user survey.
  \item \textbf{How Test Will Be Performed:} Not performed. If field testing were in scope, the client would fill out the survey after the field test. Under the current plan, only unit testing and stakeholder review are conducted.
\end{itemize}
...
\begin{itemize}
  ...
  \item Hardware-specific integration tests (require actual Jetson hardware)
\end{itemize}
...
\begin{enumerate}
  ...
  \item{test-state-management-arm-disarm\\}
        ...
        How test will be performed: Automated via test\_state\_management\_class.py::TestArmDisarm

  \item{test-state-management-manual-move\\}
        ...
        How test will be performed: Automated via test\_state\_management\_class.py::TestManualMove

  \item{test-state-management-manual-move-clamped\\}
  ...
  \item{test-state-management-recording\\}
        ...
        How test will be performed: Automated via test\_state\_management\_class.py::TestRecording

  \item{test-state-management-download-pause\\}
        ...
        How test will be performed: Automated via test\_state\_management\_class.py::TestDownloadHandlers

  \item{test-bounding-box-collection\\}
...
\begin{enumerate}
  ...
  \item{test-api-endpoints-exist\\}

        Type: Functional, Dynamic, Automatic

        Initial State: Flask test client with mocked StateManagement

        Input: HTTP requests to /api/arm, /api/disarm, /api/manual\_move, /api/manual\_move\_to, /api/recordings/start, /api/recordings/stop
        ...
        How test will be performed: Automated via test\_api.py and test\_api\_gateway.py

  \item{test-api-recordings-crud\\}
        ...
        How test will be performed: Automated via test\_api.py::TestRecordingsList, TestRecordingsRename, TestRecordingsDelete

  \item{test-api-validation\\}
...
\begin{enumerate}
  ...
  \item{test-database-allocate\\}
        ...
        How test will be performed: Automated via test\_database.py::TestAllocateRecording

  \item{test-database-list-recordings\\}
        ...
        How test will be performed: Automated via test\_database.py::TestListAllRecordings

  \item{test-database-rename-delete\\}
        ...
        How test will be performed: Automated via test\_database.py::TestRenameRecording, TestDeleteRecording

  \item{test-database-path-traversal-protection\\}
  ...
  \item{test-database-space-usage\\}
...
\begin{enumerate}
  ...
  \item{test-gimbal-packet-construction\\}
        ...
        How test will be performed: Automated via test\_gimbal.py::TestCreateRequestData

  \item{test-gimbal-angle-commands\\}
        ...
        How test will be performed: Automated via test\_gimbal.py::TestSetDeg, TestGetDeg

  \item{test-gimbal-led-control\\}
...
\begin{enumerate}
  ...
  \item{test-tracking-queue-behavior\\}
  ...
  \item{test-tracking-worker-control-law\\}
        ...
        Output: gimbal.set\_deg() called with corrected angles based on proportional control

        Test Case Derivation: Verifies FR-7 (tracking control law)

        How test will be performed: Automated via test\_tracking.py::TestWorkerControlLaw

  \item{test-tracking-angle-clamping\\}
...
\begin{enumerate}
  ...
  \item{test-cv-management-osd-send\\}
        ...
        How test will be performed: Automated via test\_cv\_process\_mgmt.py::TestSendOsdData

  \item{test-cv-management-recording-commands\\}
        ...
        How test will be performed: Automated via test\_cv\_process\_mgmt.py::TestRecordingCommands

  \item{test-cv-management-pause-resume\\}
        ...
        How test will be performed: Automated via test\_cv\_process\_mgmt.py::TestPauseResume

  \item{test-cv-management-recv-loop\\}
        ...
        How test will be performed: Automated via test\_cv\_process\_mgmt.py::TestRecvLoop
...
\begin{enumerate}
  ...
  \item{test-livestream-construction\\}
  ...
  \item{test-livestream-process-loop\\}
...
\begin{enumerate}
  ...
  \item{test-transcode-routes-404\\}
        ...
        How test will be performed: Automated via test\_transcode\_api.py::TestTranscodeRoutes

  \item{test-transcode-streaming\\}
  ...
  \item{test-transcode-pipe-cleanup\\}
        ...
        How test will be performed: Automated via test\_transcode\_api.py::TestCleanupPipe
...
\begin{enumerate}
  ...
  \item{test-transcode-read-log\\}
        ...
        How test will be performed: Automated via test\_transcode\_main.py::TestTranscodeProcessReadLog

  \item{test-transcode-shader-probe\\}
        ...
        How test will be performed: Automated via test\_transcode\_main.py::TestTranscodeProcessShaderProbe

  \item{test-transcode-bus-call\\}
        ...
        How test will be performed: Automated via test\_transcode\_main.py::TestTranscodeProcessBusCall
...
\begin{enumerate}
  ...
  \item{test-bounding-box-rotate-90\\}
  ...
  \item{test-dataclass-construction\\}
...
\begin{enumerate}
  ...
  \item{test-ipc-buffer-sender\\}
        ...
        How test will be performed: Automated via test\_ipc\_buffer.py::TestIPCBufferSender

  \item{test-ipc-buffer-receiver\\}
        ...
        How test will be performed: Automated via test\_ipc\_buffer.py::TestIPCBufferReceiver
...
\begin{enumerate}
  ...
  \item{test-watchdog-refresh\\}
        ...
        How test will be performed: Automated via test\_watchdog.py::TestWatchdogRefresh

  \item{test-watchdog-clear\\}
...
\begin{enumerate}
  ...
  \item{test-control-page-bbox-overlay\\}
...
\begin{enumerate}
  ...
  \item{test-controls-card-render\\}
        ...
        How test will be performed: Automated via test\_components/ControlsCard.test.tsx

  \item{test-control-page-drag\\}
...
\begin{enumerate}
  ...
  \item{test-recordings-page-loading\\}
  ...
  \item{test-recordings-page-empty\\}
  ...
  \item{test-recordings-page-list\\}
...
\begin{enumerate}
  ...
  \item{test-language-selector\\}
        ...
        How test will be performed: Automated via test\_components/LanguageSelector.test.tsx

  \item{test-configuration-menu\\}
        ...
        How test will be performed: Automated via test\_components/ConfigurationMenu.test.tsx
...
\begin{enumerate}
  ...
  \item{test-api-client-automatic-discovery\\}
...
\begin{enumerate}
  ...
  \item{test-rocam-provider-initialization\\}
  ...
  \item{test-rocam-provider-error-handling\\}
  ...
  \item{test-rocam-provider-polling\\}
...
\begin{enumerate}
  ...
  \item{test-formatters-temperature\\}
...
\begin{enumerate}
  ...
  \item{test-state-status-performance\\}
        ...
        How test will be performed: Automated via test\_state\_management\_class.py::TestStatus

  \item{test-state-error-handling\\}
...
\begin{enumerate}
  ...
  \item{test-sse-broadcast-performance\\}
        ...
        Test Case Derivation: Verifies SLR-3, RFR-4 (real-time status streaming without backpressure)

        How test will be performed: Automated via test\_sse.py::TestMessageAnnouncer

  \item{test-sse-thread-safety\\}
...
\begin{enumerate}
  ...
  \item{test-ipc-buffer-overrun-handling\\}

        Type: Nonfunctional, Robustness, Automatic

        Initial State: IPCBufferReceiver with head significantly ahead of tail (overrun condition)
  ...
  \item{test-ipc-buffer-memory-safety\\}
...
\begin{enumerate}
  ...
  \item{test-cv-management-exception-safety\\}
        ...
        How test will be performed: Automated via test\_cv\_process\_mgmt.py::TestExceptionHandling
...
\begin{enumerate}
  ...
  \item{test-database-error-handling\\}
...
\begin{enumerate}
  ...
  \item{test-watchdog-timeout-accuracy\\}
...
\begin{enumerate}
  ...
  \item{test-system-status-caching\\}
  ...
  \item{test-system-status-callback-updates\\}
        ...
        How test will be performed: Automated via test\_system\_status.py::TestSystemStatusMonitor
...
\begin{enumerate}
  ...
  \item{test-scheduler-validation\\}
        ...
        How test will be performed: Automated via test\_utils.py::TestSchedulerValidation
...
\begin{enumerate}
  ...
  \item{test-state-management-recording-only-noops\\}

        Type: Functional, Dynamic, Automatic

        Initial State: StateManagementRecordingManagementOnly instance

        Input: Calls to arm(), disarm(), manual\_move(), manual\_move\_to(), start\_recording(), stop\_recording()
        ...
        How test will be performed: Automated via test\_state\_management\_recording\_only.py::TestStateManagementRecordingOnly

  \item{test-state-management-recording-only-status\\}
        ...
        Output: StatusResponse with is\_recording=False, armed=False, system metrics populated

        Test Case Derivation: Verifies FR-10 (status reporting in recording-only mode)

        How test will be performed: Automated via test\_state\_management\_recording\_only.py::TestStateManagementRecordingOnly

  \item{test-control-process-main-entry\\}
        ...
        How test will be performed: Automated via test\_state\_management\_recording\_only.py::TestControlProcessMain
...
\begin{enumerate}
  ...
  \item{test-main-dispatch-control\\}
        ...
        How test will be performed: Automated via test\_main\_entry.py::TestMainDispatch

  \item{test-main-dispatch-cv\\}
  ...
  \item{test-main-dispatch-livestream\\}
  ...
  \item{test-main-dispatch-transcode\\}
  ...
  \item{test-main-dispatch-unknown\\}
...
\begin{enumerate}
  ...
  \item{test-formatters-edge-cases\\}
  ...
  \item{test-formatters-internationalization\\}
...
\begin{enumerate}
  ...
  \item{test-ui-loading-states\\}
        ...
        How test will be performed: Automated via component test files (e.g., test\_pages/control.test.tsx)

  \item{test-ui-error-handling\\}
...
\begin{longtable}{|p{4.5cm}|p{10cm}|}
  ...
  \hline
  \endlastfoot
  \mref{mState} (State Management) & test-state-management-arm-disarm; test-state-management-manual-move; test-state-management-manual-move-clamped; test-state-management-recording; test-state-management-download-pause; test-state-status-performance; test-state-error-handling \\
  \hline
  \mref{mAPI} (API Gateway) & test-api-endpoints-exist; test-api-recordings-crud; test-api-validation; test-api-error-responses \\
  \hline
  \mref{mRecord} (Recording Database) & test-database-allocate; test-database-list-recordings; test-database-rename-delete; test-database-path-traversal-protection; test-database-space-usage; test-database-error-handling \\
  \hline
  \mref{mGimbal} (Gimbal Abstraction) & test-gimbal-crc8; test-gimbal-packet-construction; test-gimbal-angle-commands; test-gimbal-led-control; test-gimbal-info-query \\
  \hline
  \mref{mTrack} (Tracking) & test-tracking-queue-behavior; test-tracking-worker-control-law; test-tracking-angle-clamping; test-tracking-stop \\
  \hline
  \mref{mCVManagement} (CV Process Management) & test-cv-management-osd-send; test-cv-management-recording-commands; test-cv-management-pause-resume; test-cv-management-recv-loop; test-cv-management-exception-safety \\
  \hline
  \mref{mVideoManagement} (Live Video Management) & test-livestream-construction; test-livestream-process-loop \\
  \hline
  \mref{mTranscodeManagement} (Transcode Management) & test-livestream-construction; test-livestream-process-loop \\
  \hline
  \mref{mTranscode} (Transcode Process) & test-transcode-read-log; test-transcode-shader-probe; test-transcode-bus-call \\
  \hline
  \mref{mCV} (CV Process) & test-cv-format-time; test-cv-update-osd \\
  \hline
  \mref{mVideo} (Live Video Process) & test-livestream-construction; test-livestream-process-loop \\
  \hline
  ...
  \hline
  Transcode API (\mref{mAPI}) & test-transcode-routes-404; test-transcode-streaming; test-transcode-pipe-cleanup \\
  \hline
  SSE (\mref{mAPI}) & test-sse-broadcast-performance; test-sse-thread-safety \\
\end{longtable}
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{4.5cm}|p{10cm}|}
    ...
    \textbf{Module} & \textbf{Unit Test Cases} \\
    \hline
    \mref{mPreview} (Preview) & test-control-page-render; test-control-page-bbox-overlay \\
    \hline
    \mref{mManual} (Manual Control) & test-controls-card-render; test-control-page-drag \\
    \hline
    \mref{mRecordMng} (Recording Management) & test-recordings-page-loading; test-recordings-page-empty; test-recordings-page-list; test-recordings-delete; test-recordings-rename \\
    \hline
    \mref{mConfig} (Configuration) & test-language-selector; test-configuration-menu \\
    \hline
...
Container modules (\mref{mJetson}, \mref{mControl}, \mref{mUI}) are also not directly tested as they are organizational constructs; their leaf modules are tested individually.
